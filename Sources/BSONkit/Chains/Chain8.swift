...
where S0 : Sequence, S1 : Sequence, S2 : Sequence, S3 : Sequence, S4 : Sequence, 
    ...
    init(_ s0: S0, _ s1: S1, _ s2: S2, _ s3: S3, _ s4: S4, _ s5: S5, _ s6: S6, _ s7: S7) {
        self.s0 = s0
        self.s1 = s1
        self.s2 = s2
        self.s3 = s3
    ...
    init<T0, T1, T2, T3, T4, T5, T6, T7>(
        ...
        T4.Encoded == S4, T5.Encoded == S5, T6.Encoded == S6, T7.Encoded == S7 {
            s0 = try t0.encode()
            s1 = try t1.encode()
            s2 = try t2.encode()
            s3 = try t3.encode()
    ...
    func makeIterator() -> Iterator {
        Iterator(s0, s1, s2, s3, s4, s5, s6, s7)
    }
}
