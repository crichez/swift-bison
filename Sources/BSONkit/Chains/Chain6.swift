...
struct Chain6<S0, S1, S2, S3, S4, S5>: Sequence 
where S0 : Sequence, S1 : Sequence, S2 : Sequence, 
      S3 : Sequence, S4 : Sequence, S5 : Sequence,
    ...
    init(_ s0: S0, _ s1: S1, _ s2: S2, _ s3: S3, _ s4: S4, _ s5: S5) {
        self.s0 = s0
        self.s1 = s1
        self.s2 = s2
        self.s3 = s3
    ...
    init<T0, T1, T2, T3, T4, T5>(_ t0: T0, _ t1: T1, _ t2: T2, _ t3: T3, _ t4: T4, _ t5: T5) throws
    where T0 : BinaryConvertible, T1 : BinaryConvertible, T2 : BinaryConvertible, 
          ...
          T0.Encoded == S0, T1.Encoded == S1, T2.Encoded == S2, T3.Encoded == S3, T4.Encoded == S4,
          T5.Encoded == S5 {
        s0 = try t0.encode()
        s1 = try t1.encode()
        s2 = try t2.encode()
